...
### 2.3 反馈与举报处理
统一收集并处理用户反馈，是维护平台内容质量和社区氛围的关键环节。

**处理流程：**
1. **接收与分类**：管理员在后台的“举报中心”看到所有状态为 `pending` 的举报列表，系统自动按“抄袭”、“垃圾广告”、“不当内容”等理由分类。
2. **状态管理**：`pending` (待处理) -> `processing` (处理中) -> `resolved` (已解决) / `archived` (已存档)。
3. **联动处置**：管理员点击某条举报，可一键跳转到被举报内容、举报人主页、被举报人主页，进行交叉验证。

#### 场景：处理抄袭举报

| 步骤 | 管理员操作 | 系统支持与数据操作 |
|------|------------|------------------|
| 1 | **审查举报** | 查看举报详情，包括被举报内容、举报人提供的证据链接和说明。 | `READ report` (with all related entities) |
| 2 | **内容比对** | 打开被举报内容和证据链接，进行人工比对，判断抄袭行为是否属实。 | - |
| 3 | **执行处置 (情况属实)** | - **内容层面**: 将被举报内容设为“私密”或直接删除。<br>- **活动层面**: 若内容已参与活动，则取消其参赛资格。<br>- **用户层面**: 对抄袭者执行警告、禁言或封禁账号等处罚。 | - `UPDATE post SET visibility = 'private'`<br>- `DELETE event:post`<br>- `UPDATE user SET status = 'banned'` |
| 4 | **关闭举报** | 将举报状态更新为 `resolved`，并选择一个处理结果（如“抄袭属实，已处理”）。 | `UPDATE report` (status: resolved) |
| 5 | **发送通知** | 系统自动向举报人发送通知：“您举报的 {内容} 经核实存在违规，已处理。感谢您的贡献。” | `CREATE notification` |
| 6 | **执行处置 (情况不属实)** | 将举报状态更新为 `resolved`，理由为“证据不足”或“非违规内容”。举报人也会收到相应通知。 | `UPDATE report` (status: resolved) + `CREATE notification` |
...
管理员对平台的活动拥有最高的管理和策展权限。

**核心操作：**

| 操作 | 对应 Schema/Logic | 说明 |
|------|-------------------|------|
| **活动审批** | `UPDATE event SET status = 'published'` | 审核组织者提交的新活动，决定是否发布。 |
| **编辑审批** | `UPDATE event SET status = 'published'` (from `pending_update`) | 审核组织者对已发布活动的修改请求（如延期），批准后变更生效。 |
| **展示策展** | `UPDATE event SET is_featured = true/false` | **人工选择**任意状态的活动（进行中、已结束等）在平台首页或特定板块进行推荐展示。 |
