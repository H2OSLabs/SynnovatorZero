...
## 7.1 内容举报 (Content Reporting)

用户在发现涉嫌抄袭、违规或不当内容时，可以向平台发起举报。

| 步骤 | 用户操作 | 系统行为与数据操作 | 说明 |
|------|----------|--------------------|------|
| 1 | **发起举报** | 在目标内容（帖子、提案、评论）的“更多”菜单中，点击“举报”按钮。 | - |
| 2 | **选择理由** | 在弹出的举报窗口中，选择一个预设的举报理由（如“抄袭”、“垃圾广告”、“不当内容”等）。 | `CREATE report` (status: pending) | 系统创建一个 `report` 对象，关联到被举报内容和举报人。 |
| 3 | **提供证据 (可选)** | 用户可以在文本框中填写详细说明，或附上证明抄袭的外部链接。 | `UPDATE report` (with details/evidence) | - |
| 4 | **提交举报** | 点击“提交”按钮。 | 系统向管理员后台的“举报中心”推送一条新任务。 | 提交后，用户会收到“举报已提交，感谢您的反馈”的提示。 |
