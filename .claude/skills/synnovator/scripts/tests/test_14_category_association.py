#!/usr/bin/env python3
"""
TC-STAGE-*, TC-TRACK-*, TC-PREREQ-*, TC-CATREL-*: Category association tests.
...
class TestCategoryAssociation(TestRunner):
    ...
    def test_tc_prereq_002_register_after_closed(self):
        ...
        # Close bounty
        update_content(self.data_dir, "category", bounty["id"], {"status": "closed"}, current_user=alice)
    ...
    # --- Category relation validation tests ---

    def test_tc_catrel_900_duplicate_rejected(self):
        """TC-CATREL-900: Duplicate category_category rejected."""
        print("\n=== TC-CATREL-900: Duplicate Rejected ===")
        alice = self.ids["user_alice"]

        cat_a = create_content(self.data_dir, "category", {
            "name": "Cat A", "description": "A", "type": "competition", "status": "published"
        }, current_user=alice)
        cat_b = create_content(self.data_dir, "category", {
    ...
    def test_tc_catrel_902_circular_dependency(self):
        ...
        s1 = create_content(self.data_dir, "category", {
            "name": "Circ1", "description": "C1", "type": "competition", "status": "published"
        }, current_user=alice)
        s2 = create_content(self.data_dir, "category", {
            "name": "Circ2", "description": "C2", "type": "competition", "status": "published"
        }, current_user=alice)
        s3 = create_content(self.data_dir, "category", {
