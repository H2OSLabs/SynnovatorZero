...
| category:category | `(source_category_id, target_category_id)` | 同一对活动间不能重复建立关联 |
...
>
> **自引用限制：** `user:user` 和 `category:category` 关系禁止 source 与 target 为同一记录（不能关注自己、活动不能关联自己）。
>
> **循环依赖检测：** `category:category` 关系中 `relation_type=stage` 和 `prerequisite` 禁止循环依赖（A→B→C→A），需在应用层通过图遍历校验。
...
| 操作 | 级联行为 |
|------|---------|
| 软删除 category | 解除所有 category:rule、category:post、category:group、category:category（作为 source 或 target）关系；通过 target:interaction 关系查找并软删除关联的 interaction，同时解除 target:interaction 关系 |
| 软删除 post | 解除所有 post:post、post:resource、category:post 关系；通过 target:interaction 关系查找并软删除关联的 interaction，同时解除 target:interaction 关系 |
...
| category:category.source_category_id | category.id | 级联软删除时解除 |
| category:category.target_category_id | category.id | 级联软删除时解除 |
