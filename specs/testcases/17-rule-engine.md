...
Rule 的 `checks` 包含 `{ condition: { type: exists, params: { entity: category_post, scope: user, filter: { relation_type: submission }, require: false } } }`（要求用户尚无提交）。用户无现有 submission。条件通过。

**TC-ENGINE-008：field_match 条件 — 字段匹配**
Rule 的 `checks` 包含 `{ condition: { type: field_match, params: { entity: category, target: $target, field: status, op: "==", value: "published" } } }`。目标活动 status 为 published。条件通过。
...
创建 Rule 时设置 `max_submissions=2`，不定义 `checks`。创建 `category_post` 时，引擎自动将 `max_submissions` 展开为 `{ trigger: create_relation(category_post), phase: pre, condition: { type: count, ... } }` 并执行校验。用户已提交 2 次，第 3 次被拒绝。
...
Rule 的 `checks` 包含：`{ trigger: update_content(category.status), phase: post, condition: { type: field_match, params: { field: status, op: "==", value: closed } }, action: compute_ranking }`。将活动从 draft 更新为 published（非 closed）。post-hook 的 condition 不满足，`compute_ranking` action 不执行。
...
Rule 的 `checks` 为 `[]`，无任何固定约束字段。活动关联该 Rule 后，创建 category_post、group_user 等操作均无约束，正常通过。
