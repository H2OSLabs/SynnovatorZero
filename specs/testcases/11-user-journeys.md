...
3. 团队报名活动（创建 event:group 关系）
4. 创建参赛帖子并关联到活动（创建 event_post）— 引擎自动校验关联 rule 的约束（时间窗口、团队人数、提交次数、格式等），全部满足后关联成功
...
- 创建日常帖子（type=general），发布后公开可见（不受 rule 约束，未关联 event）
- 创建参赛提案（type=proposal），关联到活动（event:post submission）— 引擎自动校验该活动关联的 rule 约束（时间窗口、格式、提交次数、团队人数），全部满足后关联成功
- 帖子附带 Markdown 正文和 tags
- 若帖子关联了 event 且 rule 设置 `allow_public=false`，发布需走 `pending_review` 审核路径
...
删除一个关联了 event:post（submission）、post:post（embed + reference）、post:resource（attachment + inline）以及多条 interaction（like + comment + rating）的帖子。删除后：
- 帖子被物理删除
- 所有关系被解除
