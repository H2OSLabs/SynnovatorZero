...
删除一个已关联 rule（event:rule）、post（event:post）、group（event:group）和 interaction 的活动。删除完成后：
- 活动记录被物理删除
- event:rule、event:post、event:group 关系均被解除
...
一个 participant 角色的用户尝试创建活动。系统拒绝操作，返回权限不足错误（CREATE event 仅限 Organizer/Admin）。
