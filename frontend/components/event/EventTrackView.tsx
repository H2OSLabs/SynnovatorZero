...
interface CategoryTrackViewProps {
  ...
  onCategorySelect?: (event: CategoryTrack) => void
  className?: string
}
...
export function CategoryTrackView({
  ...
  const [events, setCategories] = useState<CategoryTrack[]>([])
  ...
  const filteredCategories = selectedStage === 'all'
    ? events
    : events.filter(c => c.stage === selectedStage)

  const stageCounts = events.reduce((acc, c) => {
            ...
            >
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <CategoryStageView stage={event.stage} variant="badge" />
                </div>
              </CardHeader>
              <CardContent>
                {event.description && (
                ...
                <div className="flex items-center gap-4 text-sm text-nf-muted">
                  <span className="flex items-center gap-1">
                    <span>👥</span>
                    <span>{event.participant_count} 参与</span>
                  </span>
                </div>
              </CardContent>
            </Card>
