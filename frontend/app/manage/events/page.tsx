...
import { getCategories, Event } from '@/lib/api-client'
        ...
        <table className="w-full">
          ...
          <tbody>
            {events.length === 0 ? (
              ...
              events.map((event) => (
                <tr key={event.id} className="border-b border-nf-dark-bg last:border-0 hover:bg-nf-dark-bg/50">
                  <td className="p-4">
                    <div className="font-medium text-nf-white">{event.name}</div>
                    <div className="text-sm text-nf-muted truncate max-w-xs">
                      {event.description}
                    </div>
                  </td>
                  <td className="p-4 text-nf-light-gray">
                    {event.type === 'competition' ? '竞赛' : '运营活动'}
                  </td>
                  <td className="p-4">{getStatusBadge(event.status)}</td>
                  <td className="p-4 text-nf-light-gray">{event.participant_count}</td>
