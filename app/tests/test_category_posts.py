"""Category:Post relation tests — Phase 6 Layer 4
...
def _create_category(client, uid, name="Contest"):
...
def test_filter_by_relation_type(client):
    ...
    client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p1, "relation_type": "submission"})
    client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p2, "relation_type": "reference"})
    h = {"X-User-Id": str(uid)}
    resp = client.get(f"/api/categories/{cat_id}/posts", params={"relation_type": "submission"}, headers=h)
...
def test_list_all_category_posts(client):
    ...
    client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p1, "relation_type": "submission"})
    client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p2, "relation_type": "reference"})
    resp = client.get(f"/api/categories/{cat_id}/posts", headers={"X-User-Id": str(uid)})
    assert resp.status_code == 200
...
def test_max_submissions_enforced(client):
    ...
    # Associate rule to category
    client.post(f"/api/categories/{cat_id}/rules", json={"rule_id": rule_id, "priority": 0})
    # First submission succeeds
    p1 = _create_post(client, uid, "First Submission")
    resp1 = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p1, "relation_type": "submission"})
    assert resp1.status_code == 201
    # Second submission by same user fails
    p2 = _create_post(client, uid, "Second Submission")
    resp2 = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p2, "relation_type": "submission"})
...
def test_reference_not_limited_by_max_submissions(client):
    ...
    resp = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": p2, "relation_type": "reference"})
    assert resp.status_code == 201


def test_duplicate_category_post_rejected(client):
    """Duplicate category:post association rejected (409)."""
    uid = _create_user(client)
    cat_id = _create_category(client, uid)
    post_id = _create_post(client, uid)
    resp1 = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": post_id, "relation_type": "submission"})
    assert resp1.status_code == 201
    resp2 = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": post_id, "relation_type": "submission"})
    assert resp2.status_code == 409


def test_remove_category_post(client):
    """Remove category:post relation."""
    uid = _create_user(client)
    cat_id = _create_category(client, uid)
    post_id = _create_post(client, uid)
    resp = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": post_id, "relation_type": "submission"})
    relation_id = resp.json()["id"]
    del_resp = client.delete(f"/api/categories/{cat_id}/posts/{relation_id}")
    assert del_resp.status_code == 204
    # List should be empty
    list_resp = client.get(f"/api/categories/{cat_id}/posts", headers={"X-User-Id": str(uid)})
    assert len(list_resp.json()) == 0


def test_nonexistent_category_rejected(client):
    """Associate post to nonexistent category returns 404."""
    uid = _create_user(client)
    post_id = _create_post(client, uid)
    resp = client.post("/api/categories/9999/posts", json={"post_id": post_id, "relation_type": "submission"})
    assert resp.status_code == 404


def test_nonexistent_post_rejected(client):
    """Associate nonexistent post to category returns 404."""
    uid = _create_user(client)
    cat_id = _create_category(client, uid)
    resp = client.post(f"/api/categories/{cat_id}/posts", json={"post_id": 9999, "relation_type": "submission"})
    assert resp.status_code == 404
