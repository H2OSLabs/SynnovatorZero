"""Event:Group relation tests — Phase 6 Layer 4

Covers:
- TC-REL-CG-001: Team registers for event
...
def _create_event(client, uid, name="Competition"):
...
def test_unregister_team(client):
    """TC-REL-CG-003: Team unregisters from event."""
    uid = _create_user(client)
    cat_id = _create_event(client, uid)
    group_id = _create_group(client, uid)
    client.post(f"/api/events/{cat_id}/groups", json={"group_id": group_id})
    resp = client.delete(f"/api/events/{cat_id}/groups/{group_id}")
    assert resp.status_code == 204
    # List should be empty
    list_resp = client.get(f"/api/events/{cat_id}/groups")
...
def test_duplicate_registration_rejected(client):
    ...
    cat_id = _create_event(client, uid)
    group_id = _create_group(client, uid)
    resp1 = client.post(f"/api/events/{cat_id}/groups", json={"group_id": group_id})
    assert resp1.status_code == 201
    resp2 = client.post(f"/api/events/{cat_id}/groups", json={"group_id": group_id})
    assert resp2.status_code == 409


def test_register_nonexistent_category(client):
    """Register team to nonexistent event returns 404."""
    uid = _create_user(client)
    group_id = _create_group(client, uid)
    resp = client.post("/api/events/9999/groups", json={"group_id": group_id})
    assert resp.status_code == 404


def test_register_nonexistent_group(client):
    """Register nonexistent group to event returns 404."""
    uid = _create_user(client)
    cat_id = _create_event(client, uid)
    resp = client.post(f"/api/events/{cat_id}/groups", json={"group_id": 9999})
    assert resp.status_code == 404
